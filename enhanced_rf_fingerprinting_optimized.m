...
for localSNR = SNRList
    for localFramesPerRouter = FramesPerRouter
        for k = kValues
            ...
            % Create layer graph with proper connectivity
            lgraph = layerGraph();
            ...
            % ResNet Block 1
            tempLayers = [
                convolution2dLayer([3 1], 64, 'Padding', [1 0], 'Name', 'res1_conv1')
                batchNormalizationLayer('Name', 'res1_bn1')
                reluLayer('Name', 'res1_relu1')
                convolution2dLayer([3 1], 64, 'Padding', [1 0], 'Name', 'res1_conv2')
                batchNormalizationLayer('Name', 'res1_bn2')
                ];
            lgraph = addLayers(lgraph, tempLayers);
            
            % Skip connection for ResNet Block 1
            tempLayers = [
            ...
            % ResNet Block 2
            tempLayers = [
                convolution2dLayer([3 1], 128, 'Padding', [1 0], 'Stride', [2 1], 'Name', 'res2_conv1')
                batchNormalizationLayer('Name', 'res2_bn1')
                reluLayer('Name', 'res2_relu1')
                convolution2dLayer([3 1], 128, 'Padding', [1 0], 'Name', 'res2_conv2')
                batchNormalizationLayer('Name', 'res2_bn2')
                ];
            lgraph = addLayers(lgraph, tempLayers);
            
            % Skip connection for ResNet Block 2 (with dimension matching)
            tempLayers = [
                convolution2dLayer([1 1], 128, 'Stride', [2 1], 'Name', 'res2_skip')
                batchNormalizationLayer('Name', 'res2_skip_bn')
                ];
            lgraph = addLayers(lgraph, tempLayers);
            ...
            % Global Average Pooling and Feature Processing
            tempLayers = [
                globalAveragePooling2dLayer('Name', 'gap')
                fullyConnectedLayer(256, 'Name', 'fc_features')
                reluLayer('Name', 'relu_features')
                dropoutLayer(0.2, 'Name', 'dropout_features')
                ];
            lgraph = addLayers(lgraph, tempLayers);
            
            % Self-Attention Mechanism (Simplified)
            tempLayers = [
                fullyConnectedLayer(256, 'Name', 'attention_weights')
            ...
            % LSTM Processing (Reshape for sequence)
            tempLayers = [
                fullyConnectedLayer(frameLength, 'Name', 'reshape_for_lstm')
                functionLayer(@(X) reshape(X, [frameLength, 1, 1, size(X,4)]), 'Name', 'reshape_lstm')
                sequenceInputLayer(frameLength, 'Name', 'lstm_input')
                lstmLayer(128, 'OutputMode', 'sequence', 'Name', 'lstm1')
                dropoutLayer(0.3, 'Name', 'lstm_dropout1')
                lstmLayer(64, 'OutputMode', 'last', 'Name', 'lstm2')
                dropoutLayer(0.3, 'Name', 'lstm_dropout2')
                ];
            lgraph = addLayers(lgraph, tempLayers);
            ...
            lgraph = connectLayers(lgraph, 'maxpool1', 'res1_conv1');
            lgraph = connectLayers(lgraph, 'maxpool1', 'res1_add/in1');
            lgraph = connectLayers(lgraph, 'res1_bn2', 'res1_add/in2');
            lgraph = connectLayers(lgraph, 'res1_relu2', 'res2_conv1');
            lgraph = connectLayers(lgraph, 'res1_relu2', 'res2_skip');
            lgraph = connectLayers(lgraph, 'res2_bn2', 'res2_add/in1');
            lgraph = connectLayers(lgraph, 'res2_skip_bn', 'res2_add/in2');
            lgraph = connectLayers(lgraph, 'res2_relu2', 'gap');
            lgraph = connectLayers(lgraph, 'dropout_features', 'attention_weights');
            lgraph = connectLayers(lgraph, 'dropout_features', 'attention_apply/in1');
            lgraph = connectLayers(lgraph, 'attention_softmax', 'attention_apply/in2');
            lgraph = connectLayers(lgraph, 'attention_apply', 'reshape_for_lstm');
            lgraph = connectLayers(lgraph, 'reshape_lstm', 'lstm_input');
            lgraph = connectLayers(lgraph, 'lstm_dropout2', 'fc_final');
            ...
            tic
            fprintf('Training optimized hybrid model...\n');
